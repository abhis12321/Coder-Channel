...
export default function ToggleMode() {
  ...
  return (
    <form className="w-[45px] aspect-square mx-auto lg:mx-4 mt-4 lg:my-0 rounded-full cursor-pointer bg-white dark:bg-inherit hover:bg-gray-500 dark:hover:bg-gray-400/30 flex items-center justify-center shadow-[0_0_2px_gray_inset] overflow-hidden text-gray-500 dark:text-white hover:text-white hover:scale-110 duration-500" onSubmit={handleToggleMode}>
      <button className={`h-full w-full text-[1.4rem] ${!themeRef?.current?.classList?.contains("dark") && "hidden"}`} name='dark'>
        <FontAwesomeIcon icon={faSun} className='h-[23px] ' />
      </button>
      <button className={`h-full w-full text-[1.6rem] ${themeRef?.current?.classList?.contains("dark") && "hidden"}`} name='light'>
