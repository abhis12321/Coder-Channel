    ...
    <div className='flex gap-[1px] flex-col items-center justify-around w-[98%] mx-[2%] max-w-[400px] bg-blue-950 p-2 fixed'>
      ...
      </div>
      {
        followings?.map((user , index) => 
          <div className="flex justify-between items-center p-2 font-mono bg-blue-100/10 rounded w-full hover:bg-green-400/10"  key={index}>
            <Link href={`/students/${user.followedToId}`} className="flex-1">{user.followedToName}</Link>
