  ...
  return (
    <div className='w-[100%] relative flex items-center justify-around'>
      ...
      <div className={`w-1/3 py-[11px] rounded-sm text-white hover:bg-gray-600/15 dark:hover:bg-white/10 cursor-pointer flex gap-4 items-center justify-center font-semibold group ${option === 1 && "bg-gray-600/20 dark:bg-white/10"}`}onClick={() => setOption(prev => prev === 1 ? 0 : 1)} >
        <FontAwesomeIcon size='sm' icon={faComment} className={`h-5 group-hover:drop-shadow-none drop-shadow-[0_0_1px_black] group-hover:text-blue-700/80 ${option === 1 && "text-blue-700/80 drop-shadow-none"}`}/>
        <div className={`text-gray-500/90 group-hover:text-blue-700/90 ${option === 1 && "text-blue-700/90 drop-shadow-none"}`}>Comment</div>
      </div>
