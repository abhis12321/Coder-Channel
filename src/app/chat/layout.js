                    ...
                    <div className="w-full h-full flex justify-center relative bg-red400">
                        <div className="max-h-nav w-[40%] h-[100%] md:h-auto max-w-[300px] absolute z-10 md:static top-0 -left-full flex flex-col gap-[1px] bg-blue-50 dark:bg-blue-800/30 dark:text-white shadow-[-1px_-.5px_1px_gray_inset] dark:shadow-[-1px_-.5px_1px_white_inset] duration-500 backdrop-blur-md md:backdrop-blur-none new-scroll overflow-x-hidden" ref={listRef}>
                            <Link href="/chat" className='w-full py-[9px] px-1 md:px-2 rounded flex gap-2 items-center bg-gray-600/20 dark:bg-black/20 hover:bg-violet-700/40 shadow-[0_0_1px_gray_inset] group'>
                                ...
                                <span className="flex flex-col group-hover:underline font-bold text-lg text-nowrap">Public Group</span>
                            </Link>
                            {senders?.map(sender => <Link href={`/chat/${sender?._id}`} key={sender?._id} className='w-full py-[2px] px-1 md:px-2 rounded flex gap-2 items-center bg-gray-600/20 dark:bg-black/20 hover:bg-cyan-700/50 dark:hover:bg-cyan-600/15 shadow-[0_0_1px_gray_inset] hover:shadow-red-700 hover:shadow-sm group'>
                        ...
                        <button className="flex md:hidden absolute top-6 left-4 z-10 font-bold active:text-orange-600 drop-shadow-[0_0_2px_black] bg-green-600 rounded-3xl px-[1px]" onClick={handleUsersList} onBlur={hideUsersList}><FontAwesomeIcon size='sm' icon={faCommentDots} className='h-7' ref={menuRef} /></button>
                        {
                            children
                        }
                    </div>
