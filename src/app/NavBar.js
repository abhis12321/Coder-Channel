...
export default function NavBar() {
  ...
  return (
    <div className="bg-gradient-to-b from-slate-950 to-slate-800  text-white flex items-center justify-between h-4rem leading-[4rem] px-8 ">
      ...
      <div className="fixed top-0 right-0 lg:static lg:flex gap-1 hidden right-navbar bg-gradient-to-b from-slate-950 to-slate-800  lg:bg-inherit min-h-[100vh] lg:min-h-fit">
        ...
        <div className="hover:bg-slate-950 text-center"><Link href="/" className="py-3 px-8 rounded-lg font-bold hover:bg-slate-950" onClick={e => addActive('hom')}>
          Home
        </Link></div>
        <div className="hover:bg-slate-950 text-center"><Link href="/projects" className="py-3 px-8 rounded-lg font-bold hover:bg-slate-950" onClick={e => addActive('hom')}>
          Projects
        </Link></div>
        <div className="hover:bg-slate-950 text-center"><Link href="/chat" className="py-3 px-8 rounded-lg font-bold hover:bg-slate-950" onClick={e => addActive('hom')}>
          Chat
        </Link></div>
        <div className="hover:bg-slate-950 text-center"><Link href="/students" className="py-3 px-8 rounded-lg font-bold hover:bg-slate-950" onClick={e => addActive('stu')}>
          Students
        </Link></div>
        {!USER.user && (
          <div className="hover:bg-slate-950 text-center"><Link href="/login" className="py-3 px-8 rounded-lg font-bold hover:bg-slate-950" onClick={e => addActive('log')}>
            Login
          </Link></div>
        )}
        {USER.user && (
          <>
            <div className="hover:bg-slate-950 text-center"><Link href={`/login`} className="py-3 px-8 rounded-lg font-bold hover:bg-slate-950"  onClick={e => addActive('lg')}>
              {USER.user.name}
            </Link></div>
          </>
        )}
